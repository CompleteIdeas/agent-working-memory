...
**The current numbers are in [`benchmarks-current.md`](benchmarks-current.md).** That page is
regenerated by `npm run bench` and stamped with the version, commit, snapshot and flags that
produced it. Nothing is carried forward into it by hand.

This page is the durable half: what each suite measures, which flags are recommended, and the
measurement caveats that keep being rediscovered.

**Version history lives in [`../CHANGELOG.md`](../CHANGELOG.md).** Four instrument defects have
been found in this project — a runner using k=7 against the shipped k=3, wall-clock decay ageing
a supposedly frozen snapshot, a runner hardcoding `agentId: 'work'`, and a 400-char rerank
truncation — and each is recorded against the release that corrected it. Superseded readings are
deliberately *not* kept here: a benchmark page full of retired numbers is how someone ends up
quoting the wrong one.

One rule survives all four of those defects: **a number without its version and its corpus is
not quotable.**

## Reproducing

```bash
npm run bench           # the real-store retrieval fixtures
npm run bench -- --all  # plus the four local challenge suites
```

`bench` runs each suite in a fresh process, stamps the output with version and commit, and
archives raw stdout under `bench-runs/`. It refuses to start below 8 GB of commit headroom,
because these suites load ONNX models natively and five earlier runs were OOM-killed, one
mid-write; `BENCH_MIN_HEADROOM_GB` overrides that if you accept the risk.

The individual challenge suites can also be run directly:
...
**Do not run the real-store fixtures bare.** `tsx tests/realstore-eval/runner.ts` defaults to
the *baseline* arm — no `AWM_RERANK2`, no `AWM_RERANK_WINDOW=query`, no `AWM_RERANK_TAGS`, and
the full `fixture.json` rather than the 300-query identifier sample. Invoked that way it reports
roughly 56–64% where the recommended configuration reports low nineties, which reads as a
regression rather than a different configuration. `bench` sets the flags and the fixture. There
is deliberately no one-line npm alias, because npm scripts cannot set those env vars portably on
Windows without another dependency, and an alias that silently measured the baseline is the
exact defect class listed above.

Each eval creates a fresh SQLite database, seeds it, and runs structured challenges. No external
services required.

## Recommended retrieval flags

These three are **default-OFF** and are what `bench` measures with:
...
| Flag | What it does |
|---|---|
| `AWM_RERANK2=1` | Second-stage cross-encoder rerank. No extra inference. |
| `AWM_RERANK_WINDOW=query` | Reranks a query-centred window instead of the leading 400 chars. |
| `AWM_RERANK_TAGS=1` | Puts tags into the rerank passage, where the ranker can see them. |
...
**Quote the denominator — there are four, and they differ by an order of magnitude.**

| number | what it is |
|---|---|
| **29,853** | every row in `tests/realstore-eval/snapshot/store.db` |
| **11,337** | `stage='active'` |
| **11,294** | `stage='active' AND NOT retracted` — the figure usually quoted for this corpus |
| **11,262** | the same, also excluding superseded rows |
| **1,881** | the fixture's candidate pool — **what the runner prints as `corpus N engrams`** |

The last one is the trap: it is the number on screen when you run the benchmark, and it is not
the store size. By agent the snapshot is 22,391 `work` / 2,586 `personal`, the rest scattered
across per-directory UUID spaces. Pick one, say which, use the same one everywhere.

**LoCoMo was retired in 0.13.x** and is not coming back: passages median **115 chars** against a
real store's **1,965**; seeded in one shot, so decay, Hebbian weights and salience reinforcement
contribute nothing; no supersession, no staging history, no cross-session use; it **rewards
indiscriminate retention**, so the salience filter — the product — caps its score no matter how
good ranking gets; and it is structurally blind to the 400-char rerank truncation that affects
**79%** of real ground-truth identifiers. `tests/realstore-eval/` replaces it: real corpus, real
query register, mechanically verifiable truth, abstention scored as a positive.

**The Memory Gauntlet's probes are not deterministic.** The gauntlet (`memory-working-agent`,
`src/gauntlet/`) is the standing end-to-end acceptance test: a single-factor ablation holding
harness, model, tasks and budgets constant while only the memory substrate varies
(`awm` / `off` / `notes` / `longctx` / `rag`). At 10 reps per arm, **6 of 10 probes flip between
identical runs** and per-rep totals span 4 to 9 out of 10. An earlier 7-rep sample suggested only
6 probes were unstable, which understated it — short samples make flaky probes look solid. The
consequence is standing: **harness variance is larger than the retrieval effects being looked
for**, so a null result from the gauntlet is not evidence that a retrieval change did nothing.
Making the probes deterministic is the prerequisite for using it as a gate; more reps will not
fix it.

**`multihop` measures the agent, not the memory.** The probe asks *"What is the codename of the
project owned by my scheduler?"* — a chain of scheduler → Sarah Chen → owns Cygnus → codename
Falcon, seeded across separate tasks so no single memory contains the answer. It has been read as
an AWM capability gap and it is not: it is a measure of whether the agent chains its lookups.

## Eval Details

What each suite measures. **The pass rates quoted in this section are illustrative of what the
suite reports, not a current measurement** — `npm run bench -- --all` regenerates self-test, edge
cases, stress and consolidation with a version and commit attached, and those land in
[`benchmarks-current.md`](benchmarks-current.md). The remaining suites have no surviving run
record and are due a stamped re-run.
