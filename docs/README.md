...
| **[pipeline-walkthrough.html](pipeline-walkthrough.html)** | **The mental model** — every stage is a proposer or a decider, organised as the three questions recall asks. Rendered on the docs site |
...
| Doc | What it covers |
|---|---|
| **[desktop.md](desktop.md)** | **Claude Desktop** — the `.mcpb` extension, the shared store, and why the bundle is 4.6 KB rather than 350 MB |
...
| **[benchmarks-current.md](benchmarks-current.md)** | **The current numbers** — generated by `npm run bench`, stamped with version, commit, snapshot and flags. Never hand-edited |
| [benchmarks.md](benchmarks.md) | What each eval suite measures, the recommended flags, and the measurement caveats. Carries no results — those are in the generated page above, and version history is in the CHANGELOG |
...
- **[benchmarks-current.md](benchmarks-current.md)** — the current numbers, generated and stamped.
